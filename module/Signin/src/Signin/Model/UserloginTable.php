    ...
    public function __construct(TableGateway $tableGateway)
	 {
		 ...
		 public function signinUser($signin)
		 {
		 	...
		 	*/
		 	...
		 	if(!$row) // if useremail is not found in the  table return 0.
		 		return array('status'=>0,'msg'=>'user doesnt exist');
		 	else
		 	{
		 		$passnew= substr($password,-3);
			 	$hash=  md5($password.$passnew);
		  		...
		  		if($hash == $row->password) // if email match with the row in the table match the password with the hash generated.
		 			return array('status'=>1,'id'=>$row->user_id);  // return the user id after valid user name and password.
		 		else
		 			return array('status'=>0,'msg'=>'wrong password');
		 	}
		 ...
		 public function fetchAll()
		 {
			 ...
			 public function savenewuser(UserloginModel $userreg)
			 {
                ...
                if (($id=$this->getUserIdbyEmail($userreg))!=0)
			 ...
			 public function setemailkey($userid,$key)
			 {
			 	/* To generate the salt, first generate enough random bytes. Because
			 	 * base64 returns one character for each 6 bits, the we should generate
			 	* at least 22*6/8=16.5 bytes, so we generate 22. Then we get the first
			 ...
			 public function checkemailkey($userid,$key)
			 {
			 	/* To generate the salt, first generate enough random bytes. Because
			 	 * base64 returns one character for each 6 bits, the we should generate
			 	* at least 22*6/8=16.5 bytes, so we generate 22. Then we get the first
			 	...
			 	*/
			 	...
			 	if($row->emailkey==$key)
			 	{
			 		$this->changestatusadmin($userid, 'Y');
			 		return 1;
			 	}
			 	return 0;
